...
pb_PrintTimerSet(struct pb_TimerSet *timers)
{
  ...
  for(i = 1; i < pb_TimerID_LAST-1; ++i) { // exclude NONE and OVRELAP from this format
    if(pb_GetElapsedTime(&t[i]) != 0) {
      ...
      printf("%-*s: %f us\n", maxCategoryLength, categories[i-1], pb_GetElapsedTime(&t[i])*1e6);
      ...
      if (timers->sub_timer_list[i] != NULL) {
        ...
        // Print SubTimers
        while (sub != NULL) {
          printf(" -%-*s: %f us\n", maxSubLength, sub->label, pb_GetElapsedTime(&sub->timer)*1e6);
          sub = sub->next;
        }
      }
    }
  ...
  if(pb_GetElapsedTime(&t[pb_TimerID_OVERLAP]) != 0)
    printf("CPU/Kernel Overlap: %f us\n", pb_GetElapsedTime(&t[pb_TimerID_OVERLAP])*1e6);
        
  // float walltime = (wall_end - timers->wall_begin)/ 1e6;
  float walltime = wall_end - timers->wall_begin;
  printf("Timer Wall Time: %f us\n", walltime);  
