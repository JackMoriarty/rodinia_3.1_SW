...
int main()
{
    ...
    #pragma acc parallel loop copyout(b, x)
    for ( i = 0; i < N; i++) {
        b[i] = 0.0;
        x[i] = 0.0;
    }
    b[N-1] = (double) (N + 1);
    for (it = 0; it < M; it++) {
        #pragma acc parallel loop copyin(b, x) copyout(xnew)
        for (i = 0; i < N; i++) {
            xnew[i] = b[i];
            if (0 < i) {
                xnew[i] = xnew[i] + x[i - 1];
        ...
        d = 0.0;
        #pragma acc parallel loop reduction(+:d) copyin(x, xnew)
        for (i = 0; i < N; i++) {
            d = d + pow(x[i] - xnew[i], 2);
        }

        #pragma acc parallel loop copyin(xnew) copyout(x)
        for (i = 0; i < N; i++) {
            x[i] = xnew[i];
        }

        r = 0.0;
        #pragma acc parallel loop reduction(+:r) copyin(b, x)
        for (i = 0; i < N; i++) {
            t = b[i] - 2.0 * x[i];
            if (0 < i) {
                t = t + x[i - 1];
