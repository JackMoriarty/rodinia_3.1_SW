...
unsigned int h_graph_mask[NO_OF_NODES];
unsigned int h_updating_graph_mask[NO_OF_NODES];
unsigned int h_graph_visited[NO_OF_NODES];
...
void BFSGraph()
{
    ...
    do {
        ...
        #pragma acc parallel loop copy(h_graph_mask) copyin(h_graph_nodes) cache(h_graph_visited:16, h_graph_edges:16) annotate(readonly(h_graph_visited, h_graph_edges))
        for (tid = 0; tid < NO_OF_NODES; tid++) {
            if (h_graph_mask[tid] != 0) {
                h_graph_mask[tid] = 0;
                for (i = h_graph_nodes[tid].starting; i < \
