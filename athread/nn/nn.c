...
int main(int argc, char* argv[]) {
	...
	while(!done) {
		...
		spe_param.rec_count = rec_count;
		spe_param.sandbox = sandbox;
		spe_param.z = z;
		spe_param.target_lat = target_lat;
		spe_param.target_long = target_long;
		// printf("%d\n", rec_count);
		athread_spawn(nn_slave, &spe_param);
		athread_join();
        // for (i = 0; i < rec_count; i++){
		// 	rec_iter = sandbox+(i * REC_LENGTH + LATITUDE_POS - 1);
	...
	double time_use = (end.tv_sec - start.tv_sec) * 1e6 + \
